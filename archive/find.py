...
file_paths = ['1_1.json','2.json', '3.json', '4.json', '5.json', '6.json', '7_1.json', '8.json', '9_1.json', '10.json', '11.json', '12.json', '13.json', '14.json', '15.json', '16.json', '17.json', '18.json', '19.json', '20.json', '21.json', '22_1.json', '23.json', '24.json', '25.json', '26.json']
start_write = True
...
for file_path in file_paths:
    ...
    # print("peak:", peaks_time)
    # print("peak:", fsr_filtered[peaks_above])
    # print("bp:", bp)
    # print("max peak = ",max(fsr_filtered[peaks_above]))
    # print("num = ",np.argmax(fsr_filtered[peaks_above]))
    # print("max bp = ",bp[np.argmax(fsr_filtered[peaks_above])])

    closest_peak_indices = []
    for time in peaks_time:
        time_diff = np.abs(np.array(peaks_above) - time)
        closest_peak_index = np.argmin(time_diff)
        closest_peak_indices.append(closest_peak_index)
    index_values = [bp[i] for i in closest_peak_indices]

    output_file_path = f"output_{file_path}"
    # age height weight BP pwts rr 
    with open('output2.txt', 'w' if start_write else 'a') as file:
        file.write(f"Age: {age}\n")
        file.write(f"Height: {height}\n")
        file.write(f"Weight: {weight}\n")
        # file.write(f"BP: {BP}\n")
        # file.write(f"test: {peaks_time}\n")
        # file.write(f"testpeak: {peaks_above}\n")
        file.write(f"index: {index_values}\n")
        # file.write(f"len: {len(pwts)}\n")
        file.write(f"PWTS: {pwts}\n")
        file.write(f"RR Intervals: {rr_intervals}\n\n")
